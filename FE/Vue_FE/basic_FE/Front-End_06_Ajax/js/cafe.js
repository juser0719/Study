    ...
    if (httpRequest1.readyState == 4) {            //서버로 부터 data를 전부 받은 상태(Completed)
        if (httpRequest1.status == 200) {          // 응답상태(OK)
            ...
            for (var i = 0; i < len; i++){
                ...
                var desc = book.querySelector("description").childNodes[0].nodeValue;
...
function essayBook() {
    if (httpRequest2.readyState == 4) {            //서버로 부터 data를 전부 받은 상태(Completed)
        if (httpRequest2.status == 200) {          //응답상태(OK)
            var books = httpRequest2.responseText; //서버에서 온 data를 text로 받음
            //alert(books);

            var booklist = JSON.parse(books);      // text를 JSON객체로 변환
            var len = booklist.length;
            var bookView = "";
            for (var i = 0; i < len; i++){
                var book = booklist[i];
