...
function programmingBook() {
    //서버로 부터 data 전부 받은 상태 
    if (httpRequest1.readyState == 4) {
        if (httpRequest1.status == 200) { // 응답 상태.
            const books = httpRequest1.responseXML; // 서버에서 온 data를 XML로 받음
            console.log(books);
            const bookList = books.querySelectorAll("book");
            let bookView = "";
            for (var i = 0; i < bookList.length; i++) {
                const book = bookList[i];
                let isbn = book.querySelector("isbn").childNodes[0].nodeValue;
                let title = book.querySelector("title").childNodes[0].nodeValue;
                let price = book.querySelector("price").childNodes[0].nodeValue;
                let desc = book.querySelector("description").childNodes[0].nodeValue;
...
function essayBook() {
    if (httpRequest2.readyState == 4) {
        if (httpRequest2.status == 200) {
            const books = httpRequest2.responseText;
